...
public class ScoreExec_06 {
	...
	public static void main(String[] args) {
		...
		for(int i =0; i< 50; i++) {
			...
			/*
			 * 어떤 클래스를 개체로 생성해서 List에 추가하고자 할때는 값들을 대입하기에 
			 * 앞서 반드시 클래스를 객체로 (새로) 생성해서 사용해야한다.
			 */
		 ...
		 *  이 List.size() 메서드는 호출할때마다 매번 List의 개수를 계산한다.
		 *  따라서 배열의 length와는 성질이 달라서 for문 내에서 가급적 
		 *  사용하지 말고 for가 시작되기전에 미리 변수에 담아놓고
		 *  for 반복문을 실행하는 것이 좋다.
		 */
		int scLen = scList.size();
		for(int i = 0; i <scLen ; i++) {

			// scList i 번째위치에 저장된 ScoreVOd의 주소를 가져와서 vo에 담아라
